...
1. Add rows only when the user explicitly confirms applying and identifies the
   ...
   microsoft, intuit. NVIDIA remains unsupported.
   For Apple, retain the original location-specific posting ID, not position_id.
   Microsoft uses the Eightfold ID, Intuit the TalentBrew posting ID and Snowflake
   the full Phenom detail-route jobSeqNo; do not substitute their alternate
   ATS/requisition IDs. Snowflake jobId is not unique across public postings.
   Rippling uses the job UUID, never a location-expanded Algolia objectID.
